...
/*   Updated: 2025/01/27 17:37:37 by jewu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_image	*get_wall_texture_rendering(t_info *info, int hori_flag)
{
	...
	if (hori_flag == 0)
	{
		if (info->ray->ray_angle > PI / 2
			&& info->ray->ray_angle < 3 * (PI / 2))
...
float	get_v_line_intersection(t_info *info, float angle)
{
	...
	pixel = check_intersection(angle, &vert_x, &x_step, false);
	...
	return (sqrt(pow(vert_x - info->player->p_x, 2)
			+ pow(vert_y - info->player->p_y, 2)));
}
...
float	get_h_line_intersection(t_info *info, float angle)
{
	...
	pixel = check_intersection(angle, &hori_y, &y_step, true);
